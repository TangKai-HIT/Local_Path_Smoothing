...
P0 = line.getPointFromEnd(2.5 * l);
P1 = line.getPointFromEnd(2 * l);
P2 = line.getPointFromEnd(l);
...
%get c1_o
a = tan(phi_e)/ l_o;
b = 1/l_o^2 * (1 - 1/cos(phi_e));
poly = [-2.5, 2*a, b, 1/20 * a]; %polynomial coefficients
c1_o_candi = roots(poly); %get roots

c1_o_candi_real = [];
for i = 1:length(c1_o_candi)
    if isreal(c1_o_candi(i))
        c1_o_candi_real = [c1_o_candi_real, c1_o_candi(i)];
    end
end

c1_o = min(c1_o_candi_real(c1_o_candi_real>0)); %choose minimum among positive roots
% c1_o = abs(c1_o_candi(2));
% c1_o = min(c1_o_candi_real(c1_o_candi_real>0));
disp("c1_o:"); disp(c1_o);
